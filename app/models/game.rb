  ...
  def self.find_game_by_input(var, params)
    ...
    Game.where("archived = ? AND published = ?", false, true).each do |t|
      if t.tags.include?(params.downcase) || t.name.include?(params.downcase)
        ...
        var << { id: t.id, name: t.name, tags: t.tags, username: username, description: t.description, published: t.published, plays: t.plays, score: t.score, created_at: t.created_at, thumbnail: t.thumbnail }
      end
    end
    var
  end
